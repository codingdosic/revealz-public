extends Node2D
## 상대 손패. 배치=HandFanLayout (정적 부채꼴, 상단 미러).
## 튜닝: scripts/ui/hand_fan_layout.gd 의 OPPONENT_* 상수.
...
func _prune_hand() -> void:
	opponent_hand = opponent_hand.filter(func(c): return c != null and is_instance_valid(c))
...
func add_card_to_hand(card, speed) -> void:
	...
	CardHelpers.prepare_for_hand(card, GameConstants.Side.OPPONENT)
	...
	if card not in opponent_hand:
		...
		var idx := opponent_hand.find(card)
		_apply_pose(card, idx, opponent_hand.size(), speed if speed != null else DEFAULT_CARD_MOVE_SPEED)


## 전 손패에 부채 포즈를 적용한다.
func update_hand_positions(speed) -> void:
	_prune_hand()
	var n := opponent_hand.size()
...
func _apply_pose(card: Node2D, index: int, count: int, speed: float) -> void:
	var pose := HandFanLayout.pose_for(
		GameConstants.Side.OPPONENT, index, count, _viewport_size()
	)
