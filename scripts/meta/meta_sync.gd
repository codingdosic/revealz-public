extends Node
## Autoload MetaSync — Acc0 부팅 후 health 게이트 · 메타 GET/이관 PUT · Store 저장 시 스냅샷 푸시.
## 로컬 파일은 캐시. 서버 meta_revision 낙관적 잠금. health 실패·점검은 온라인/상점을 막음.
## UX: 부팅·이관·진입 재점검만 LoadingGate (취소 없음). 상점/온라인은 watch로 점검 전환 감지.
...
## 부팅: health → GET 스냅샷 / 404 이관 / 실패 시 로컬 캐시(온라인인 척하지 않음).
func sync_boot() -> void:
	if _busy:
		return
	_busy = true
	...
	if status == 404 or get_err == "account_not_found" or get_err == "not_found":
		_show_gate("로컬 데이터 이관 중…")
		_ensure_local_stores()
		var put: Dictionary = await _put_current_snapshot(true)
		if bool(put.get("ok", false)):
			_apply_snapshot(put.get("data", {}) as Dictionary)
			meta_available = true
			boot_done = true
			_busy = false
			_hide_gate()
			sync_finished.emit(true, "migrated")
			print("[MetaSync] boot migrate POST ok account=%s rev=%d" % [key, meta_revision])
			return
		var put_status := int(put.get("status", 0))
		var put_err := String(put.get("error", ""))
		if put_status == 410 or put_err == "account_deleted":
			_cache_without_meta("account_deleted")
			print("[MetaSync] migrate blocked account_deleted account=%s" % key)
			return
		print("[MetaSync] migrate POST fail status=%d error=%s" % [
			put_status, put_err
		])
		_cache_without_meta("migrate_failed:%s" % put_err)
		return
...
## Store 변경 후 전체 스냅샷 PUT. meta 불가면 true(로컬만). 실패 시 false.
func push_snapshot_async() -> bool:
	...
	while true:
		_push_dirty = false
		var put: Dictionary = await _put_current_snapshot(false)
...
## 로컬 캐시가 비어 있지 않게 Store를 올린다(시드 가능).
...
## 현재 메모리·디스크 기준으로 PUT body 구성 후 전송.
func _put_current_snapshot(mark_migrated: bool) -> Dictionary:
	_ensure_local_stores()
	var body := build_snapshot_from_local()
	if mark_migrated:
		body["markMigrated"] = true
	var key := AccountService.current_id()
	return await MetaRemote.put_snapshot(_http, key, body)


## 로컬 Store → API 스냅샷 Dictionary.
func build_snapshot_from_local() -> Dictionary:
	WalletStore.ensure_loaded()
	CollectionStore.ensure_loaded()
	AccessoryStore.ensure_loaded()
	var owned_payload: Dictionary = {}
	var counts := CollectionStore.all_counts_by_rarity()
	for id in counts.keys():
		var by_rarity: Dictionary = counts[id]
		...
		if rarity_payload.is_empty():
			continue
		owned_payload[str(int(id))] = rarity_payload
	var decks: Array = []
	...
	return {
		...
		"gold": WalletStore.get_gold(),
		"owned": owned_payload,
		"ownedAccessories": AccessoryStore.snapshot_owned(),
		"decks": decks,
...
func _apply_snapshot(data: Dictionary) -> void:
	...
	if not display.is_empty() and display != AccountService.display_name():
		AccountService.set_display_name(display)
...
## 온라인(매칭) 가능 여부. health 실패·점검 중이면 false.
func can_use_online() -> bool:
	return not online_blocked


## 상점·메타 쓰기 가능. 온라인 가능 + 메타 GET/PUT 성공.
func can_use_shop() -> bool:
	return can_use_online() and meta_available
...
## health 실패·점검: 로컬 캐시만. 온라인/상점을 막는다.
...
## 메타 API만 실패. 싱글·(health OK면) 온라인은 가능. 상점은 막힘.
...
## 서버 팩 구매. 성공 시 로컬 gold/owned 반영 후 ShopService 형식 Dictionary.
...
func purchase_pack_async(body: Dictionary) -> Dictionary:
	...
	print("[MetaSync] purchase ok spent=%s cards=%d rev=%d" % [
	...
	return {
		...
		"granted_card_ids": granted_ids,
		"granted_names": granted_names,
		"granted_rarities": granted_rarities,
	}
