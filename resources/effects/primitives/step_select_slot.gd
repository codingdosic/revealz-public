...
@export var store_key: String = "slot"
...
func run(source: Node, run_ctx: EffectPipelineRunContext) -> void:
	...
	if run_ctx.effect_manager and run_ctx.effect_manager.has_method("select_slots"):
		picked = await run_ctx.effect_manager.select_slots(slots, 1, source)
	elif slots.size() == 1:
		picked = [slots[0]]
	if picked.is_empty():
		if abort_on_fail:
			run_ctx.store("_abort", true)
		return
	var slot: CardSlot = picked[0] as CardSlot
	if not store_key.is_empty():
		run_ctx.store(store_key, slot)
